                ...
                for i in range(alignment_len):
                    ...
                    if 0 <= seq_pos < subject_length:
                        if q_char == h_char and q_char != '-':
                            ...
                            subject_array[seq_pos] = 2 if not reverse else 6
                        elif q_char == '-':
                            subject_array[seq_pos] = 3 if not reverse else 7
                        else:
                            subject_array[seq_pos] = 4
...
def count_value_occurrences(subject_array):
    ...
    return {f'n_of_{num}': counts.get(num, 0) for num in range(8)}

def process_csv(input_csv, output_csv):
    with open(input_csv, 'r') as infile, open(output_csv, 'w', newline='') as outfile:
        reader = csv.DictReader(infile)
        fieldnames = reader.fieldnames + ['n_of_0', 'n_of_1', 'n_of_2', 'n_of_3', 'n_of_4', 'n_of_5', 'n_of_6', 'n_of_7', 'binary_filename']
